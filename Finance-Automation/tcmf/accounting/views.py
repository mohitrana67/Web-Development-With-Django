...
def add_csv_data(request, *args, **kwargs):
    ...
    if request.method == 'POST' and request.FILES['uploadedFile']:  # We are cehcking if the request we are getting from the form is the POST request and the FILE field actually has something in it
        ...
        quarter = request.POST['quarter'] # getting the value of the the quarter to store in the database so that we can use it after to create a report for a particular quarter
        quarter = int(quarter)
        year = request.POST['year'] # getting year to generate reports afterwards
        docuemnt_type = request.POST['docuemnt_type']   # getting the type of document uploaded(can be Credit or Debit)
        allowed_quarter = [1,2,3,4] # creating a list of allowed quarters so that user can not enter a quarter which is not feasible
        ...
        elif quarter not in allowed_quarter:    # checking if the quarter enterd by the user is the allowed quarter
            context["message"] = "Please enter a valid quater"
        # We will be creating more validations in the future
        else:
